...
export const roleRouteGroups = {
  ...
  admin: [
    'Home', 'Profile', 'CreateWaybill', 'Success', 'OrderDetail', 'ProcessedList',
    'ScanBagging', 'ScanInHub', 'ScanManifestLoad', 'ScanManifestUnload', 'ScanTask',
    'HubManagement', 'ShopStatement', 'AdminOperations', 'StaffManagement', 'UpdateStatus',
    'WaybillList', 'PricingRules', 'AssignShipper', 'CashConfirm',
    'AccountingDashboard', 'WarehouseDashboard', 'WarehouseMenu', 'AccountantMenu',
    'CameraPOD', 'TaskList', 'TaskDetail',
  ],
  ...
  hub_manager: [
    'Home', 'Profile', 'WaybillList', 'ProcessedList',
    'CreateWaybill', 'Success', 'OrderDetail',
    'ScanInHub', 'ScanBagging', 'ScanManifestLoad', 'ScanManifestUnload',
    'AssignShipper', 'WarehouseDashboard', 'WarehouseMenu',
    'HubManagement', 'ShopStatement', 'CameraPOD',
  ],

  // ROLE 3 — Kho hàng (WAREHOUSE_STAFF):
  // Chỉ thực hiện nhiệm vụ trong kho: quét nhập kho, đóng gói, tải/gỡ manifest
  warehouse: [
    'Home', 'Profile', 'ProcessedList', 'CreateWaybill', 'Success',
    'ScanInHub', 'ScanBagging', 'ScanManifestLoad', 'ScanManifestUnload',
    'WaybillList', 'WarehouseDashboard', 'WarehouseMenu', 'CameraPOD',
  ],
  ...
  shipper: [
    'Home', 'Profile', 'ProcessedList', 'CreateWaybill', 'Success',
    'ScanTask', 'TaskList', 'TaskDetail', 'UpdateStatus', 'CameraPOD',
  ],
  ...
  accountant: [
    'Home', 'Profile', 'ProcessedList', 'CreateWaybill', 'Success',
    'ShopStatement', 'CashConfirm', 'AccountingDashboard', 'AccountantMenu',
    'WaybillList', 'CameraPOD',
  ],

  // ROLE 6 — Kinh doanh (SALES): Xem báo cáo, tra cứu đơn hàng
  sales: [
    'Home', 'Profile', 'ProcessedList', 'CreateWaybill', 'Success', 'ShopStatement', 'WaybillList', 'CameraPOD',
  ],

  // Mặc định: Người dùng chưa xác định vai trò
  default: [
    'Home', 'Profile', 'ProcessedList', 'CreateWaybill', 'Success', 'CameraPOD',
  ],
};
...
export const getRoleKey = (user) => {
  ...
  const normalizedRole = String(user?.role_name || user?.role || '').toUpperCase();
...
const allMenuItems = [
  ...
  { route: 'CreateWaybill', label: 'Tạo vận đơn', icon: 'document-text-outline' },
  ...
  { route: 'ScanManifestLoad', label: 'Bốc hàng lên xe', icon: 'cloud-upload-outline' },
  { route: 'ScanManifestUnload', label: 'Dỡ hàng khỏi xe', icon: 'cloud-download-outline' },
  { route: 'WarehouseDashboard', label: 'Tổng quan kho', icon: 'speedometer-outline' },
  { route: 'WarehouseMenu', label: 'Nghiệp vụ kho', icon: 'grid-outline' },
  // --- NGHIỆP VỤ GIAO HÀNG ---
  { route: 'ScanTask', label: 'Quét mã nhiệm vụ', icon: 'qr-code-outline' },
  { route: 'TaskList', label: 'Danh sách nhiệm vụ', icon: 'list-circle-outline' },
  { route: 'UpdateStatus', label: 'Cập nhật trạng thái đơn', icon: 'refresh-circle-outline' },
  { route: 'CameraPOD', label: 'Chụp ảnh xác nhận (POD)', icon: 'camera-outline' },
  // --- QUẢN LÝ BƯU CỤC ---
  { route: 'AssignShipper', label: 'Phân công người giao hàng', icon: 'person-add-outline' },
  { route: 'HubManagement', label: 'Quản lý bưu cục', icon: 'business-outline' },
  { route: 'ShopStatement', label: 'Đối soát cửa hàng', icon: 'cash-outline' },
  { route: 'WaybillList', label: 'Danh sách vận đơn', icon: 'clipboard-outline' },
  // --- KẾ TOÁN ---
  { route: 'CashConfirm', label: 'Chốt ca / Thu tiền COD', icon: 'wallet-outline' },
  { route: 'AccountingDashboard', label: 'Tổng quan tài chính', icon: 'stats-chart-outline' },
  { route: 'AccountantMenu', label: 'Nghiệp vụ kế toán', icon: 'pie-chart-outline' },
  // --- QUẢN TRỊ VIÊN ---
  { route: 'PricingRules', label: 'Bảng giá vận chuyển', icon: 'calculator-outline' },
  { route: 'StaffManagement', label: 'Quản lý nhân sự', icon: 'people-circle-outline' },
  { route: 'AdminOperations', label: 'Quản trị hệ thống', icon: 'build-outline' },
];
