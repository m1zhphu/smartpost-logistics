...
@router.get("/users", tags=["Master Data"])
def get_all_users(db: Session = Depends(get_db)):
    """Trả về danh sách user nhưng ẩn mật khẩu"""
    users = crud_auth.get_all_users(db)
    return [
        {"user_id": u.user_id, "username": u.username, "full_name": u.full_name, "role_id": u.role_id} 
        for u in users
    ]